...
class AvaDataset(Dataset):
    """
    Dataset for training a language model with conversational dataset like ChatLM.
    It tokenizes the input texts and prepares them for training.
    """
    ...
    def __init__(self, data, tokenizer, max_length=256):
        ...
        for conversation in data:
            if (
                len(conversation) < 2
                or "content" not in conversation[0]
                or "content" not in conversation[1]
            ):
                continue
            ...
            if torch.max(input_ids).item() >= len(tokenizer):
                print(
                    f"⚠ Warning: Found token ID {torch.max(input_ids).item()} which is >= vocabulary size {len(tokenizer)}"
                )
                continue
